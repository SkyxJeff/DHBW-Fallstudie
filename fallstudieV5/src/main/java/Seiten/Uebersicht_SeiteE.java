...
public class Uebersicht_SeiteE extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        nachname_vorname_label.setText(MenueE.Nachname+ ", " + MenueE.Vorname );
    ...
    public String StundenUebersichtHeuteIst() {
        ...
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fallstudie", "root", "");
            String Beginn = ("Select Beginn,Pause,Ende from eintraege Where Datum = (SELECT MAX(Datum) FROM eintraege WHERE Mitarbeiter_ID = '"+LoginE.username+"') AND Mitarbeiter_ID = '"+LoginE.username+"';");
            String maxDatum = ("Select max(Datum) from `Eintraege` where `Mitarbeiter_ID` = '"+LoginE.username+"';");
            ...
            if(Date.equals(aktuellesDatum)) {
                ...
                if(pause.equals("0,0")) {
                    ...
                    if((int)heuteIst1 > 9) {
                        ...
                        JOptionPane.showMessageDialog(null,"You are working too much. Please take longer breaks.");
                    ...
                    String pauseEintragen6h = "Update eintraege set Pause = '"+pausezeit+"' where Datum = '"+aktuellesDatum+"' and Mitarbeiter_ID = '"+LoginE.username+"';";
            ...
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Entry failed.4");

        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "You've logged in for the first time, immediately save an entry under 'My working hours'.");
            try {
                Class.forName("com.mysql.jdbc.Driver");
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fallstudie", "root", "");
                LocalDate Datumaktuell = LocalDate.now();
                String DateAktuell = ""+Datumaktuell;
                String sql6 = ("Insert into eintraege (`Mitarbeiter_ID`, `Datum`, `Beginn`, `Pause`, `Ende`, `Art`,`KW`,`Quartal`, `Jahr`, `Saldo`) values ('"+LoginE.username+"','"+DateAktuell+"','00:00','0,0','00:00','-',0,0,0,0);");
            ...
            // TODO Auto-generated catch block
            JOptionPane.showMessageDialog(null, "Entry failed.5");
            e.printStackTrace();
        }
        return leer;
    }
	...
	public void insertSaldo(float Saldo) {
		...
		try {
			...
			String updateSaldo = "Update eintraege set Saldo = '"+Saldo+"'Where Datum = '"+maxDate+"' AND Mitarbeiter_ID = '"+LoginE.username+"';";
...
//			int QuartalInt = rsQuartal.getInt(1);
            String SaldoSummeQuartal = "Select sum(Saldo) from eintraege where Quartal = '"+Quart+"' AND Mitarbeiter_ID = '"+LoginE.username+"' AND Datum Like '"+calendar.get(Calendar.YEAR)+"-__-__' ;";
...
//			int KWInt = rsKW.getInt(1);
            String SaldoSummeKW = "Select sum(Saldo) from eintraege where Datum LIKE '"+calendar.get(Calendar.YEAR)+"-"+calendar.get(Calendar.MONTH)+"-__' AND Mitarbeiter_ID = '"+LoginE.username+"';";
...
//		int JahrInt = rsJahr.getInt(1);
            String SaldoSummeJahr = "Select sum(Saldo) from eintraege where Jahr = '"+Year+"' AND Mitarbeiter_ID = '"+LoginE.username+"';";
	...
	public void PopUp() {
        ...
        try {
            Class.forName("com.mysql.jdbc.Driver");
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fallstudie", "root", "");
            String Beginn = ("Select Beginn,Pause from eintraege Where Datum = (SELECT MAX(Datum) FROM eintraege WHERE Mitarbeiter_ID = '"+LoginE.username+"') AND Mitarbeiter_ID = '"+LoginE.username+"';");
            java.sql.PreparedStatement pst1 = con.prepareStatement(Beginn);
            ResultSet rs = pst1.executeQuery();
            rs.next();
            String ArbZGmax = "Select ArbZGmax from mitarbeiter where Mitarbeiter_ID = '"+LoginE.username+"';";
            ...
            if(beginnhh < 6) {
                ...
                if( heuteistvor6float >maxArbZG ) {
                    ...
                    JOptionPane.showMessageDialog(null, "You've exceeded your daily working hours (ArbZG), contact your supervisor.");
                }
