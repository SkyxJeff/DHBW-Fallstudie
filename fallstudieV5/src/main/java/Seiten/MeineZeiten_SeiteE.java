...
public class MeineZeiten_SeiteE extends javax.swing.JPanel {
    ...
    private void speichern_button1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_speichern_button1ActionPerformed
		...
		try {
			...
			String Ende = ("Select `Ende` from eintraege where Datum = (SELECT MAX(Datum) FROM eintraege WHERE Mitarbeiter_ID = '"+LoginE.username+"') AND Mitarbeiter_ID = '"+LoginE.username+"';");
			String maxDatum = ("SELECT MAX(Datum),count(EintraegeNr) FROM eintraege WHERE Mitarbeiter_ID = '"+LoginE.username+"';");
			String age  = "Select Age from mitarbeiter where Mitarbeiter_ID = '"+LoginE.username+"';";
			...
			String DatenAusEintraege = ("Select Datum from eintraege where Mitarbeiter_ID = '"+LoginE.username+"';");
			...
			if(/*EndeUhrzeit.equals("00:00") && */ DateAktuell.equals(eingDatumEngl)) {
				for(int y = 0; y<i ; y++) {
					...
					if(DatumausCount.equals(eingDatumEngl)) {
						...
						String sql7 = ("Update eintraege set Beginn = '"+beginn_textfeld.getText()+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");							String sql3 = ("Update eintraege set Pause = '"+pause_textfeld.getText()+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
						String sql4 = ("Update eintraege set Ende = '"+ende_textfeld.getText()+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
						String sql8 = ("Update eintraege set KW = '"+kalenderWoche+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
						String sql9 = ("Update eintraege set Quartal = '"+quartal+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
						String sql10 = ("Update eintraege set Jahr = '"+Jahrpublic+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
						String sql11 = ("Update eintraege set Art = '"+art_combobox.getSelectedItem()+"'where Datum = '"+Date+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
				...
				String sql5 = ("Insert into eintraege (`Mitarbeiter_ID`, `Datum`, `Beginn`, `Pause`, `Ende`, `Art`,`KW`,`Quartal`, `Jahr`, `Saldo`) values ('"+LoginE.username+"','"+eingDatumEngl+"','"+beginn_textfeld.getText()+"','"+pause_textfeld.getText()+"','"+ende_textfeld.getText()+"','"+art_combobox.getSelectedItem()+"',"
						+ "'"+kw+"','"+Quartal+"','"+jahr+"','0');");
				...
				for(int y = 0; y<i ; y++) {
					...
					if(DatumausCount.equals(eingDatumEngl)) {
						...
						if((WorkTimeUpdateFloat >9 && PauseZeitUpdate < 0.75 && Age >= 18 ) || (WorkTimeUpdateFloat >6 && PauseZeitUpdate < 0.5 && Age >= 18) || (WorkTimeUpdateFloat >6 && PauseZeitUpdate < 1 && Age < 18) || (WorkTimeUpdateFloat <6 && WorkTimeUpdateFloat > 4.5 && PauseZeitUpdate < 0.5 && Age < 18)) {
							JOptionPane.showMessageDialog(null, "You didn't take your correct amount of breaks, please ask your supervisor about your break times (ArbZG).\n Your entries weren't submitted, please look after your health.");
						}else {
							String sql7 = ("Update eintraege set Beginn = '"+beginn_textfeld.getText()+"'where Datum = '"+eingDatumEngl+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
							String sql4 = ("Update eintraege set Ende = '"+ende_textfeld.getText()+"'where Datum = '"+eingDatumEngl+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
							String sql3 = ("Update eintraege set Pause = '"+pause_textfeld.getText()+"'where Datum = '"+eingDatumEngl+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
							...
							String sql11 = ("Update eintraege set Art = '"+art_combobox.getSelectedItem()+"'where Datum = '"+eingDatumEngl+"'AND Mitarbeiter_ID = '"+LoginE.username+"';");
							java.sql.PreparedStatement pst3 = con.prepareStatement(sql3);
							java.sql.PreparedStatement pst4 = con.prepareStatement(sql4);
							java.sql.PreparedStatement pst7 = con.prepareStatement(sql7);
//						java.sql.PreparedStatement pst8 = con.prepareStatement(sql8);
...
//						pst10.executeUpdate();
							pst11.executeUpdate();
							String NachtragBeginnEnde = "Select Beginn, Ende, Pause from eintraege where Datum = '"+eingDatumEngl+"'AND Mitarbeiter_ID = '"+LoginE.username+"';";
							...
							String maxArbeitszeit = "Select maxArbeitszeitproTag from taetigkeit_bez where Bezeichnung = (Select Taetigkeit from Mitarbeiter where Mitarbeiter_ID = '"+LoginE.username+"');";
							...
							System.out.println("End" + endeDate);
							System.out.println("Start" + beginnDate);
							...
							System.out.println("Summary without breaks: "+ SaldoNachtragFloat);
							SaldoNachtragFloat = SaldoNachtragFloat - maxArbeitszeitFloat  - pauseFloat;
							System.out.println("With breaks: " +SaldoNachtragFloat);
							String updateSaldo = "Update eintraege set Saldo = '"+SaldoNachtragFloat+"'Where Datum = '"+eingDatumEngl+"' AND Mitarbeiter_ID = '"+LoginE.username+"';";
...
//				}
			...
			JOptionPane.showMessageDialog(null, "Entry failed.1");
			loeschen_button1ActionPerformed(evt);
			e.printStackTrace();
		} catch (SQLException e) {
			JOptionPane.showMessageDialog(null, "Entry failed.2");
...
public void Urlaub(){
	try {
		Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/fallstudie", "root", "");
			String Urlaubsanspruch = ("SELECT Urlaubsanspruch FROM `urlaub` WHERE MitarbeiterID = '" + LoginE.username + "'");
			String Urlaub_aktuelles_Jahr = ("SELECT UrlaubaktuellesJahr FROM `urlaub` WHERE MitarbeiterID = '" + LoginE.username + "'");
			String Urlaub_vorjahr = ("SELECT Urlaubvorjahr FROM `urlaub` WHERE MitarbeiterID = '" + LoginE.username + "'");
			String Urlaub_genommen = ("SELECT Urlaubgenommen FROM `urlaub` WHERE MitarbeiterID = '" + LoginE.username + "'");
			String Urlaub_verfuegbar = ("SELECT Urlaubverfuegbar FROM `urlaub` WHERE MitarbeiterID = '" + LoginE.username + "'");
    ...
    private void speichern_button2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_speichern_button2ActionPerformed
    	try {
	    	...
	    	String eingabe = ("INSERT INTO `abwesendheit`(`MitarbeiterID`, `Datum`, `Beginn`, `Ende`, `Tage`, `Grund`, `Notiz`) VALUES ('"+LoginE.username+"','"+DateAktuell+"','"+Abwesendheitsbeginn+"','"+Abwesendheitsende+"','"+tage+"','"+abwesenheitsgrund_combobox.getSelectedItem()+"','"+Notiz+"')");
			...
			JOptionPane.showMessageDialog(null, "Days are not working.");
		}
